...
// ── Normalize type for comparison (handles "land", "LAND", "Land" etc.)
...
const getFallbackImage = (type: string, id: string) => {
  ...
  return pool[parseInt(id, 10) % pool.length]
}
  ...
  const fetchData = async () => {
    try {
      ...
      const formattedProperties: Property[] = (propertiesRes.data || []).map((prop: any) => ({
        ...
        image: prop.image || getFallbackImage(prop.type || "Apartment", prop.id),
      ...
      const openPoolsData: OpenPool[] = (poolsRes.data || [])
        .filter((pool: any) => pool.isPublic === true && pool.status === "open")
        .map((pool: any) => {
          ...
          return {
            ...
            name: pool.name,
            propertyTitle: pool.propertyTitle || "Property",
            propertyLocation: pool.propertyLocation || "Location",
            propertyType: normalizeType(pool.propertyType || "Apartment"),
            propertyImage: pool.propertyImage || getFallbackImage(pool.propertyType || "Apartment", pool.id),
            targetAmount: pool.targetAmount,
            raisedAmount: pool.raisedAmount,
            remainingStake: remaining,
            remainingPercentage: (remaining / pool.targetAmount) * 100,
            memberCount: pool.memberCount || 0,
            memberLimit: pool.memberLimit || 10,
    ...
    <main style={{ minHeight: "100vh", background: "#F5F5F0", fontFamily: "var(--font-body)" }}>
      ...
      <div style={{
        ...
        <div style={{ width: "100%", boxSizing: "border-box" }}>
          ...
          {/* Tabs */}
          <div style={{ display: "flex", gap: "28px", marginBottom: "20px", borderBottom: "1px solid rgba(255,255,255,0.08)", paddingBottom: "0" }}>
              ...
              <div style={{
                ...
                {filteredOpenPools.map((pool, i) => (
                  <OpenPoolCard
                    ...
                    pool={pool}
                    index={i}
                    onClick={() => router.push(`/pools/join/${pool.id}`)}
                  />
                ))}
              </div>
            )}
