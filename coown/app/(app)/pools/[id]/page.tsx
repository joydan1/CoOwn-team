...
const fetchPoolData = async () => {
  try {
    ...
    const transformedPool: Pool = {
      ...
      creatorId: data.creator?.id || data.creator_id
    }
  ...
  const handleMakePublic = async () => {
    try {
      await poolsApi.togglePublic(poolId)
        ...
        <div style={{ marginBottom: "32px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "16px" }}>
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              ...
              {/* Make Public Button - Only show for creator if pool is not already public */}
              {isCreator && !pool.isPublic && pool.status === "open" && (
                <button
